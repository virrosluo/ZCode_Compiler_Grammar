    ...
    if len(argv) < 1:
        ...
        if argv[2] == 'true':
            process1 = subprocess.Popen(
                ["java", "-jar", ANTLR_JAR,
                "-o", "test/gui",
                "main/zcode/parser/ZCode.g4", "-Dlanguage=Java"])
            process1.wait()
        ...
        process3 = subprocess.run('java -cp ".{}{}" org.antlr.v4.gui.TestRig ZCode program -gui ../testcases/{}'.format(
            seperator, ANTLR_JAR, argv[1]), cwd="{}/test/gui".format(os.getcwd()), shell=True)
